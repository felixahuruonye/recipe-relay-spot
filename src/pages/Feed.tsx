...
import { Heart, MessageCircle, Star, Eye, Volume2, VolumeX, Zap, Timer, UserPlus, ToggleLeft, ToggleRight, Plus, TrendingUp, ChevronUp, ShieldAlert, Coins } from 'lucide-react'; 
...
import AdsterraFeedCard from '@/components/Ads/AdsterraFeedCard'; 
interface Post { 
id: string; 
title: string; 
body: string; 
...
<motion.div 
className="absolute inset-0 bg-black/70 backdrop-blur-md flex items-center justify-center z-20 rounded-2xl" initial={{ opacity: 0 }} 
animate={{ opacity: 1 }} 
exit={{ opacity: 0 }} 
> 
<motion.div 
...
<div className="w-14 h-14 mx-auto rounded-full bg-yellow-500/20 flex items-center justify-center"> <ShieldAlert className="w-7 h-7 text-yellow-500" /> 
</div> 
<h3 className="font-bold text-base">Can't Earn From This Post</h3> 
<p className="text-sm text-muted-foreground"> 
This post requires <span className="font-bold text-yellow-500">{starPrice} ■</span> to watch & earn. You have <span className="font-bold">{userStars} ■</span>. 
</p> 
...
<button onClick={onDismiss} className="text-white/70 hover:text-white text-xs font-medium">OK</button> </div> 
</motion.div> 
); 
...
<motion.div 
ref={cardRef} 
className="relative bg-card rounded-2xl overflow-hidden border border-border/50 shadow-sm" initial={{ opacity: 0, y: 20 }} 
animate={{ opacity: 1, y: 0 }} 
...
<Avatar className="w-9 h-9 cursor-pointer ring-2 ring-primary/20" onClick={onProfile}> <AvatarImage src={postUser?.avatar_url} /> 
...
<span className="text-muted-foreground text-xs">· {timeAgo(post.created_at)}</span> </div> 
<div className="flex items-center gap-1.5"> 
<Badge variant="outline" className="text-[10px] h-4 px-1.5">{post.category}</Badge> {post.boosted && <Badge className="bg-orange-500 text-white text-[9px] h-4 px-1">■ Hot</Badge>} </div> 
</div> 
</div> 
<div className="flex items-center gap-1"> 
{!isOwnPost && !isFollowing && ( 
<Button size="sm" variant="outline" className="h-7 text-xs px-2 gap-1" onClick={onFollow}> <UserPlus className="w-3 h-3" /> Follow 
</Button> 
)} 
<PostMenu postId={post.id} postOwnerId={post.user_id} onPostDeleted={onFetchPosts} /> </div> 
</div> 
...
{isMuted ? <VolumeX className="w-4 h-4 text-white" /> : <Volume2 className="w-4 h-4 text-white" />} </button> 
</> 
) : ( 
<img 
src={post.media_urls[0]} 
...
<Heart className={`w-6 h-6 transition-colors ${isLiked ? 'fill-red-500 text-red-500' : 'text-muted-foreground hover:text-red-400'}`} /> <span className="text-sm font-semibold">{likesCount}</span> 
</motion.button> 
...
.on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'user_profiles', filter: `id=eq.${user.id}` }, (payload) => { const d = payload.new as any; 
...
const { data } = await supabase.from('products').select('*, user_profiles:seller_user_id(username, avatar_url, vip)').eq('status', ' active').order('created_at', { ascending: false }).limit(10);
setProducts(data || []); 
}; 
...
if (!user) return; 
const { data } = await supabase.from('user_profiles').select('star_balance, wallet_balance').eq('id', user.id).single(); setUserStarBalance(data?.star_balance || 0); 
setWalletBalance(data?.wallet_balance || 0); 
}; 
const loadCurrentUserProfile = async () => { 
if (!user) return; 
const { data } = await supabase.from('user_profiles').select('username, avatar_url').eq('id', user.id).single(); setCurrentUserProfile(data); 
}; 
const loadUserStories = async () => { 
const { data } = await supabase.from('user_storylines').select('*').gt('expires_at', new Date().toISOString()).order('created_at', { ascending: false }); 
if (data) { 
const userStoryMap = new Map(); 
data.forEach(story => { if (!userStoryMap.has(story.user_id)) userStoryMap.set(story.user_id, story); }); const userIds = Array.from(userStoryMap.keys()); 
if (userIds.length > 0) { 
const { data: profiles } = await supabase.from('user_profiles').select('id, username, avatar_url').in('id', userIds); const profileMap = new Map(profiles?.map(p => [p.id, p])); 
setStories(Array.from(userStoryMap.values()).map(story => ({ ...story, user_profile: profileMap.get(story.user_id) }))); } else { 
setStories([]); 
} 
} 
}; 
...
if (error && error.code === 'PGRST116') { setNeedsProfileSetup(true); setLoading(false); return; } 
...
const { data: postsData } = await query.order('boosted', { ascending: false }).order('rating', { ascending: false }).order('created_at', { ascending: false }).limit(50); 
let filteredPosts = postsData || []; 
if (user) { 
const { data: hiddenPosts } = await supabase.from('hidden_posts').select('post_id').eq('user_id', user.id); const hiddenPostIds = new Set(hiddenPosts?.map(h => h.post_id) || []); 
...
const { data: usersData } = await supabase.from('user_profiles').select('id, username, avatar_url, vip').in('id', userIds); const usersLookup: { [key: string]: UserProfile } = {}; 
usersData?.forEach(u => { usersLookup[u.id] = u; }); 
const { data: likesData } = await supabase.from('post_likes').select('*').in('post_id', filteredPosts.map(p => p.id)); const likesLookup: { [key: string]: PostLike[] } = {}; 
likesData?.forEach(like => { if (!likesLookup[like.post_id]) likesLookup[like.post_id] = []; likesLookup[like.post_id].push(like); }); const counts = await Promise.all(filteredPosts.map(async (p) => { 
const { count } = await (supabase as any).from('post_comments').select('*', { count: 'exact', head: true }).eq('post_id', p.id); return { id: p.id, count: count || 0 }; 
})); 
const countMap = new Map(counts.map(c => [c.id, c.count])); 
setPosts(filteredPosts.map(p => ({ ...p, comments_count: countMap.get(p.id) || p.comments_count || 0 }))); setUsers(usersLookup); 
...
const channel = supabase 
.channel('posts-feed') 
.on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'posts', filter: 'status=eq.approved' }, () => fetchPosts()) .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'post_likes' }, (payload) => { 
setPostLikes(prev => { const updated = { ...prev }; const postId = payload.new.post_id; if (!updated[postId]) updated[postId] = [];  updated[postId].push(payload.new as PostLike); return updated; }); 
setPosts(prev => prev.map(post => post.id === payload.new.post_id ? { ...post, likes_count: (post.likes_count || 0) + 1 } : post)); }) 
.on('postgres_changes', { event: 'DELETE', schema: 'public', table: 'post_likes' }, (payload) => { 
setPostLikes(prev => { const updated = { ...prev }; const postId = payload.old.post_id; if (updated[postId]) updated[postId] =  updated[postId].filter(l => l.id !== payload.old.id); return updated; }); 
setPosts(prev => prev.map(post => post.id === payload.old.post_id ? { ...post, likes_count: Math.max(0, (post.likes_count || 0) - 1) } :  post)); 
}) 
.on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'post_views' }, (payload) => { 
setPosts(prev => prev.map(post => post.id === payload.new.post_id ? { ...post, view_count: (post.view_count || 0) + 1 } : post)); }) 
.subscribe(); 
return () => { supabase.removeChannel(channel); }; 
}; 
...
const { data, error } = await supabase.rpc('process_post_view', { p_post_id: post.id, p_viewer_id: user.id }); if (error) { console.error('RPC Error:', error); return; } 
const result = data as any; 
if (result?.success) { 
setProcessedPosts(prev => new Set(prev).add(post.id)); 
await loadUserBalances(); 
if (result.charged) { toast({ title: '■ You Earned!', description: `■ ${result.stars_spent} Stars → ■${result.viewer_earn} cashback!` }); } 
else if (!result.already_viewed && result.free) { toast({ title: '■ View recorded' }); } 
} 
} finally { processingPostsRef.current.delete(post.id); } 
}, [user, processedPosts]); 
const skipToNextPost = useCallback(() => { scrollToNextPost(); }, [scrollToNextPost]); 
if (needsProfileSetup) return <ProfileSetup onComplete={() => { setNeedsProfileSetup(false); checkUserProfile(); }} />; if (needsOnboarding) return <OnboardingFlow onComplete={() => setNeedsOnboarding(false)} />; 
if (loading) return <LenoryLoader label="Loading feed..." />; 
...
const personal = ((seedFromString(p.id) ^ userSeed) % 1000) / 1000; 
const score = engagement / Math.pow(ageHrs, 0.7) + personal * 5; 
return { post: p, score }; 
}) 
.sort((a, b) => b.score - a.score) 
.map((x) => x.post); 
const feedItems: { type: 'post' | 'product' | 'suggested' | 'trending-stories' | 'ad'; data: any; key: string }[] = []; let productIndex = 0; 
...
feedItems.push({ type: 'product', data: products[productIndex], key: `product-${products[productIndex].id}` }); productIndex++; 
}
...
if ((i + 1) % 5 === 0) { 
feedItems.push({ type: 'ad', data: null, key: `ad-${i}` }); 
} 
}); 
return ( 
...
<h1 className="text-xl font-black bg-gradient-to-r from-primary to-accent bg-clip-text text-transparent">Lenory</h1> <div className="flex items-center gap-2"> 
...
<Badge variant="secondary" className="text-[10px] font-bold">■{walletBalance.toLocaleString()}</Badge> <button onClick={() => setAutoScroll(!autoScroll)} className={`p-1.5 rounded-full transition-all ${autoScroll ? 'bg-primary  text-primary-foreground shadow-lg shadow-primary/30' : 'bg-muted text-muted-foreground'}`}> 
{autoScroll ? <ToggleRight className="w-4 h-4" /> : <ToggleLeft className="w-4 h-4" />} 
</button> 
</div> 
</div> 
<div className="flex gap-1.5 px-4 pb-2 pt-1"> 
<Button variant={!showOldPosts ? 'default' : 'outline'} size="sm" className="flex-1 h-8 text-xs font-semibold" onClick={() =>  setShowOldPosts(false)}> 
<TrendingUp className="w-3 h-3 mr-1" /> New 
</Button> 
<Button variant={showOldPosts ? 'default' : 'outline'} size="sm" className="flex-1 h-8 text-xs font-semibold" onClick={() =>  setShowOldPosts(true)}> 
Past Posts 
</Button> 
<Button size="sm" className="h-8 text-xs px-3 gap-1 bg-gradient-to-r from-primary to-accent text-primary-foreground font-semibold"  onClick={() => setIsCreatePostOpen(true)}> 
...
onSelect={() => { setSelectedStoryUserId(story.user_id); setShowStoryViewer(true); }} 
/> 
))} 
</div> 
</div> 
...
<Button onClick={() => setIsCreatePostOpen(true)} className="gap-2"><Plus className="w-4 h-4" /> Create Post</Button> </div> 
) : ( 
feedItems.map((item) => { 
if (item.type === 'suggested') return <SuggestedUsers key={item.key} />; 
if (item.type === 'product') return <ProductCard key={item.key} product={item.data} />; 
if (item.type === 'trending-stories') return <TrendingStoriesCard key={item.key} />; 
if (item.type === 'ad') return <AdsterraFeedCard key={item.key} />; 
...
<motion.div className="bg-card rounded-2xl p-6 mx-6 max-w-sm w-full shadow-2xl" initial={{ scale: 0.8 }} animate={{ scale: 1 }}> <div className="text-center space-y-4"> 
...
<Button className="w-full" onClick={() => { setShowBotCheck(false); scrollTimestamps.current = []; }}>I'm Learning! ■</Button> </div> 
</motion.div> 
</motion.div> 
)} 
</AnimatePresence> 
...
<CreatePostWizard onPostCreated={fetchPosts} isOpen={isCreatePostOpen} onOpenChange={setIsCreatePostOpen} /> {showStoryViewer && selectedStoryUserId && ( 
<EnhancedStorylineViewer userId={selectedStoryUserId} open={showStoryViewer} onClose={() => { setShowStoryViewer(false);  setSelectedStoryUserId(null); }} /> 
)} 
{showCreateStory && ( 
