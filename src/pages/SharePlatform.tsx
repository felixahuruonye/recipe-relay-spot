...
const SharePlatform = () => {
  ...
  // Route referral links through /api/share-profile so social crawlers get
  // real og:image (referrer's avatar) + description; real users are
  // 302-redirected by the edge function to /?ref=CODE where the app captures
  // the ref code into localStorage for the signup flow.
  const referralUrl = user ? `${platformUrl}/api/share-profile?code=${user.id.slice(0, 8)}` : platformUrl;
  ...
  const shareToSocial = (platform: string) => {
    const shareText = profile 
      ? `Join me on Lenory Social! 🎉 Follow @${profile.username} - ${referralUrl}`
      : `Join Lenory Social! 🎉 The best social platform for earning! ${referralUrl}`;
    ...
    <div className="p-4 max-w-2xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-2xl font-bold">Share Lenory</h1>
        </div>
      </div>
          ...
          <div className="bg-gradient-to-r from-primary/20 to-primary/5 p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">Lenory Social</p>
              </div>
            </div>
          </div>
          <CardContent className="p-4">
            <p className="text-sm text-muted-foreground">
              Join me on Lenory Social! Earn while you browse, share content, and connect with amazing people.
            </p>
          </CardContent>
        </Card>
      )}
