...
declare
  ...
  v_viewer_stars bigint := 0;
  v_uploader uuid;
  v_price bigint := 0;
  v_track_id text;
  v_track_artist_id uuid;
  v_track_title text;
  uploader_share numeric := 0;
  musician_share numeric := 0;
  viewer_share numeric := 0;
...
begin
  ...
  if v_viewer_stars < v_price then
    ...
    return json_build_object('success', true, 'charged', false, 'insufficient_stars', true, 'required', v_price, 'available', coalesce(v_viewer_stars, 0), 'viewer_earn', 0, 'uploader_earn', 0, 'musician_earn', 0, 'stars_spent', 0);
  end if;

  if v_post.music_track_id is not null then
    select id, artist_id, title
    into v_track_id, v_track_artist_id, v_track_title
    from public.music_tracks
    where id = v_post.music_track_id;
  end if;

  if v_track_artist_id is not null and v_track_artist_id <> v_uploader then
  ...
  set star_balance = coalesce(star_balance, 0) - v_price
  where id = p_viewer_id;
  ...
  if musician_share > 0 then
    update public.user_profiles
    set wallet_balance = coalesce(wallet_balance, 0) + musician_share,
        total_earned = coalesce(total_earned, 0) + musician_share
    where id = v_track_artist_id;
  end if;
  ...
  if musician_share > 0 then
    insert into public.wallet_history (user_id, type, amount, currency, meta) values
      (v_track_artist_id, 'music_royalty', musician_share, 'NGN', jsonb_build_object('post_id', p_post_id, 'track_id', v_track_id, 'track_title', v_track_title, 'stars_spent', v_price, 'viewer_id', p_viewer_id::text));
  end if;
  ...
  if musician_share > 0 then
    insert into public.user_notifications (user_id, title, message, type, notification_category, related_id, action_data)
    values (v_track_artist_id, 'Music Royalty 🎵', format('You earned ₦%s from your sound!', musician_share), 'success', 'music_royalty', p_post_id, jsonb_build_object('post_id', p_post_id, 'track_id', v_track_id, 'amount', musician_share));
  end if;
...
grant execute on function public.process_post_view(text, uuid) to authenticated;